...
export function PlatformLanding() {
  ...
  const submitTournament = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setCreatedLink(`https://${cleanSlug}.liguita.co`);
    setSaving(false);
  };
  ...
  return <main className="platform">
    ...
    <section id="torneos" className="tournamentCatalog"><p className="sectionLabel">DESCUBRE LIGAS</p><h2>Campeonatos en Liguita</h2>{loading ? <p>Cargando torneos…</p> : tournaments.length ? <div className="catalogGrid">{tournaments.map(t => <a key={t.id} href={`https://${t.slug}.liguita.co`}><div style={{backgroundImage:`url(${t.image_url || "/liguita-logo-white.png"})`}}/><span>{t.status.replace("_"," ")}</span><h3>{t.name}</h3><p>{t.description || "Consulta partidos, equipos y estadísticas."}</p><small>{t.slug}.liguita.co →</small></a>)}</div> : <div className="noTournaments"><img src="/liguita-logo-google-white.png" alt=""/><h3>El próximo campeonato puede ser el tuyo</h3><button className="primaryBtn" onClick={openCreate}>Registrar mi torneo</button></div>}</section>
