...
export function MatchSheet({ id }: { id: string }) {
  ...
  const eventPlayers = [...picks.home.map(item => rosters.home.find(player => player.id === item.player_id)), ...picks.away.map(item => rosters.away.find(player => player.id === item.player_id))].filter(Boolean);
