...
export function MatchSheet({ id }: { id: string }) {
  ...
  const eventPlayers = picks[eventSide].map(item => ({ ...rosters[eventSide].find(player => player.id === item.player_id), ...item })).filter(player => player.id);
  const onFieldIds = new Set(picks[eventSide].filter(item => item.is_starter).map(item => item.player_id));
  events.filter(item => item.team_id === eventTeamId && item.event_type === "substitution").forEach(item => { if (item.player_id) onFieldIds.delete(item.player_id); if (item.secondary_player_id) onFieldIds.add(item.secondary_player_id); });
  const onFieldPlayers = eventPlayers.filter(player => onFieldIds.has(player.id));
  const substitutePlayers = eventPlayers.filter(player => !onFieldIds.has(player.id));
  ...
  return <main className="sheetPage"><header><a href={`/panel/torneos/${match.tournaments.id}`}>← Volver al torneo</a><div><span className="sectionLabel">PLANILLA OFICIAL</span><h1>{match.home.name} <b>{match.home_score} — {match.away_score}</b> {match.away.name}</h1><p>{match.scheduled_at ? new Date(match.scheduled_at).toLocaleString("es-CO") : "Sin fecha"} · {match.venue?.name || "Sin cancha"}</p></div><span className={`sheetStatus ${match.status}`}>{match.status === "in_progress" ? "En juego" : match.status === "finished" ? "Finalizado" : "Por iniciar"}</span></header>
    ...
    {(isLive || match.status === "finished") && <section className="liveSheet"><div className="liveActions"><div><span className="sectionLabel">EVENTOS DEL PARTIDO</span><h2>{match.home.name} {match.home_score} — {match.away_score} {match.away.name}</h2></div>{isLive && <div><button className="primaryBtn" onClick={() => { setEventTeamId(match.home_team_id); setEventType("goal"); setShowEvent(true); }}>+ Registrar evento</button><button className="outlineBtn" onClick={finishMatch}>Finalizar partido</button></div>}</div>{events.length ? events.map(item => <article className="matchEvent" key={item.id}><b>{item.minute}{item.stoppage_minute ? `+${item.stoppage_minute}` : ""}&apos;</b><span>{item.event_type.replaceAll("_", " ")}</span><strong>{item.event_type==="substitution"?`Sale #${item.player?.jersey_number} ${item.player?.full_name||""} → Entra #${item.secondary?.jersey_number} ${item.secondary?.full_name||""}`:`#${item.player?.jersey_number} ${item.player?.full_name||"Sin jugador"}`}</strong><small>{item.notes}</small></article>) : <p>Aún no se han registrado eventos.</p>}</section>}
    {showEvent && <div className="createOverlay"><form className="createTournament" onSubmit={addEvent}><button type="button" className="modalClose" onClick={() => setShowEvent(false)}>×</button><p className="sectionLabel">NUEVO EVENTO</p><h2>Registrar novedad</h2><div className="formPair"><label>Equipo<select name="team" required value={eventTeamId||match.home_team_id} onChange={event=>setEventTeamId(event.target.value)}>{[[match.home_team_id,match.home.name],[match.away_team_id,match.away.name]].map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label><label>Tipo<select name="type" value={eventType} onChange={event=>setEventType(event.target.value)}><option value="goal">Gol</option><option value="own_goal">Autogol</option><option value="yellow_card">Tarjeta amarilla</option><option value="red_card">Tarjeta roja</option><option value="substitution">Sustitución</option><option value="penalty_scored">Penal convertido</option><option value="penalty_missed">Penal errado</option></select></label></div><div className="formPair"><label>Minuto<input name="minute" type="number" min="0" max="180" required/></label><label>Minutos adicionales (opcional)<input name="stoppage" type="number" min="0" max="30" defaultValue="0"/><small className="fieldHelp">Ejemplo: para 45+2 escribe minuto 45 y adicionales 2.</small></label></div><label>{eventType==="substitution"?"Jugador que sale":"Jugador"}<select name="player" required><option value="">Seleccionar jugador</option>{(eventType==="substitution"?onFieldPlayers:eventPlayers).map(player=><option value={player.id} key={player.id}>#{player.jersey_number} · {player.full_name}</option>)}</select></label>{eventType==="substitution"&&<label>Jugador que entra<select name="secondary" required><option value="">Seleccionar suplente</option>{substitutePlayers.map(player=><option value={player.id} key={player.id}>#{player.jersey_number} · {player.full_name}</option>)}</select>{!substitutePlayers.length&&<small className="fieldHelp warning">No hay suplentes disponibles. Revisa que hayan sido incluidos como suplentes en la planilla.</small>}</label>}<label>Nota<textarea name="notes" rows={2}/></label><button className="primaryBtn wide">Guardar evento</button></form></div>}
  </main>;
}
